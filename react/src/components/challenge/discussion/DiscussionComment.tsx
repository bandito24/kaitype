...
import ChallengeVotes from '@/components/challenge/discussion/ChallengeVotes.tsx'
...
export default function DiscussionComment({
  ...
  const {refetch, data: replyData} = useQuery({
    queryKey: ['replies', {challengeId: challengeId, replyId: comment.id}],
    queryFn: async () => {
      const response = await loadCommentReplies(comment.id)
      return response.data.comments
    },
    enabled: false,
  })
  useEffect(() => {
    setChildComments(replyData)
  }, [replyData])
  ...
  async function handleDelete() {
    ...
    console.log(result)
    if (!result.childrenLength && comment.parent_id && setParentHasResponse) {
      console.log('no mas hijos')
      setParentHasResponse(false)
    }
  }
  ...
  return (
    <div
              ...
              <div className="relative">
                ...
                <div
                  ...
                  <ul
                    ...
                    style={{
                      display: showSelfCommentOptions ? 'block' : 'none',
                    }}>
                    <li
                      onClick={() => {
                        if (isOwnPost) {
      ...
      {errors && (
        <div className="absolute z-20 mb-[0] mt-[-15px]">
